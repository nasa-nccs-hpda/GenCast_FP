...
def proc_time_step(ds_org, ctime, ref_date, output_dir, case="init"):
    ...
    valid_rename_dict = {
        k: v for k, v in rename_dict.items() if k in ds.variables
    }
    ...
    ds.attrs = {
        "title": f"{fmodel} forecast start at {str(ref_date)}",
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Convert GenCast output to CF-compliant NetCDF"
    )
    ...
    parser.add_argument(
        ...
        "-g",
        type=str,
        help="Inputs for GenCast to get surface geopotential height",
    )
    parser.add_argument(
        "--pred_dir",
        "-p",
        type=str,
        help="directory of GenCast prediction files",
    )
    parser.add_argument(
        "--output_dir",
        "-o",
        type=str,
        help="Output directory to CF-compliant NetCDF",
    )
    ...
    for case in ["init", "pred"]:
        if case == "init":
            ...
            files = sorted(
                Path(args.geos_dir).glob(
                    f"*source-geos*{args.year}-{args.month}-{args.day}_*.nc"
                )
            )
            ref_date = np.datetime64(
                f"{args.year}-{args.month}-{args.day}T00:00:00"
            )
            n = 2  # only process first two time steps for initial conditions
        else:
            files = sorted(
                Path(args.pred_dir).glob(
                    f"*geos_date-{args.year}-{args.month}-{args.day}_*.nc"
                )
            )
            ref_date = np.datetime64(
                f"{args.year}-{args.month}-{args.day}T12:00:00"
            )
            n = -1  # process all time steps for prediction

        ds = xr.open_dataset(files[0])
        ds = ds.drop_vars("land_sea_mask", errors="ignore")
        for nt, ctime in enumerate(ds.time.values[:n]):
            print("Processing time step ", nt, "for case ", case)
            proc_time_step(
                ds, ctime, ref_date, output_dir=output_dir, case=case
            )
